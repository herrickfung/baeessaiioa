...
def read_files(master, sessions):
    #  Read Trial Info
    master = master[0]
    master = pd.read_csv(master, delimiter=',')

    #  Read Eprime session response files
    for i in range(6):
        session = sessions[i]
        session = pd.read_table(session, delimiter='\t', header=[1], encoding='utf-16')
        if i == 0:
            merged = session
        else:
...
def stat_graph(data):
    ...
    Std_Total_Beta = (data["C1_Beta"].astype(float).sem(),
                      data["C2_Beta"].astype(float).sem(),
                      data["C3_Beta"].astype(float).sem()
                       )
    print(Mean_Total_Beta)
    ...
    host.set_ylabel(r'Perceptual Sensitivity $\beta$', fontname="CMU Serif", fontsize=12, fontweight="bold")

    plt.ylim(0, 55)
    plt.yticks(np.arange(0, 55, 5))

    host.plot([0.0, 0.1, 0.2], Mean_Total_Beta, color = '#222222', marker = '^', markersize=8)
    host.errorbar([0.0, 0.1, 0.2], Mean_Total_Beta, yerr=[(0,0,0),Std_Total_Beta],
                  color='#888888', elinewidth=1,
                  capsize=2, alpha=0.8)
...
def curve_graph(data):
    ...
    for j in range(3):
        ...
        plt.subplot(3, 1, j+1).text(-22.5, 0.9, labels[j], ha='center',
                                    fontsize=14, fontname='CMU Serif',
        ...
        plt.plot(xforplot, yforplot, color = '#222222', linewidth = 2.5, alpha = 0.8)
        plt.plot(xdatapoint, ydatapoint, 'o', color = '#222222', alpha = 0.8)
    ...
    plt.gcf().text(0.5, 0.05, "Tilt (Degree in Clockwise)",
                   ha='center', fontsize=16, fontname='CMU Serif',
                   fontweight='bold')
...
# PSY402 Research Thesis II/part2_result/behavioral_stats.txt"
    ...
    for_analysis = pd.read_csv('~/Desktop/Course Materials/Sem 4.1/PSY402 Research Thesis II/experiment_part2/result/fitted_data/fit_result.csv', ',')
