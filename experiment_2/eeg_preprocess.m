...
addpath 'C:\Program Files\Polyspace\R2020a\toolbox\eeglab_current\eeglab2021.0';
eeglab;
...
SortedCorrPath = fullfile(CurrentPath, 'sorted_by_cond_and_correct_trials');
CorePaths = {PreProcessedPath, MergedPath, SortedCorrPath};
...
for i = 1 : length(Parti_List)
    ...
    ConvertPath = fullfile(InPPPath, 'behavioral', 'convert', 'converted.csv');
    OutPPFilePath = fullfile(PreProcessedPath, Parti_List(i).name);
    OutMergedPath = fullfile(MergedPath, Parti_List(i).name);
    OutSortedCorrPath = fullfile(SortedCorrPath, Parti_List(i).name);
    PartiPaths = {OutPPFilePath, OutMergedPath, OutSortedCorrPath};
    ...
    SORTEPOCH_BY_CORR(ConvertPath, OutMergedPath, OutSortedCorrPath);
end
...
function SORTEPOCH_BY_CORR(ConvertPath, OutMergedPath, OutSortedCorrPath)
    ConvertFileName = dir(ConvertPath);
    ConvertFileName = append(ConvertFileName.folder, '\', ConvertFileName.name);
    TrialList = readtable(ConvertFileName, 'Format', 'auto');
       
    trial_con = struct('single',[TrialList.Trial_No, TrialList.Condition == 1, TrialList.Response_Accuracy == 1],...
                       'lowSD', [TrialList.Trial_No, TrialList.Condition == 2, TrialList.Response_Accuracy == 1],...
                       'highSD', [TrialList.Trial_No, TrialList.Condition == 3, TrialList.Response_Accuracy == 1]...
                       );
    
    fn = fieldnames(trial_con);
    
    for n = 1:numel(fn)
        trial_con.(fn{n}) = find(trial_con.(fn{n})(:,2)==1 & TrialList.Response_Accuracy == 1);
        trial_con.(fn{n}) = reshape(trial_con.(fn{n}), 1, []);
        disp(size(trial_con.(fn{n})));
    end
    ...
    for i = 1:numel(efn)
        OutFileName = append(OutSortedCorrPath, '\', efn{i}, '.set');
        OutFileCSVName = append(OutSortedCorrPath, '\', efn{i}, '.txt');
        SortedEpochs.(efn{i}) = pop_saveset(SortedEpochs.(efn{i}), 'filename', OutFileName);
        SortedEpochs.(efn{i}) = pop_export(SortedEpochs.(efn{i}), OutFileCSVName, 'erp', 'on', 'transpose', 'on', 'separator', ',', 'precision', 6);
    end
